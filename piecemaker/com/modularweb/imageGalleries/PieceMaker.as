package com.modularweb.imageGalleries {
	...
	public class PieceMaker extends MovieClip {
		...
		private function initiate (e:Event) {
			...
			tmc.name = "textMC";
			tmc.tf_mc.tf.x = textDistance;
			tmc.tf_mc.tf.y = textDistance;
			tmc.tf_mc.tf.width = imageHeight - textDistance * 2;
			tmc.tf_mc.tf.height = imageHeight - textDistance * 2;
			tmc.tf_mc.tf.wordWrap = true;
			tmc.tf_mc.tf.mouseWheelEnabled = false;
			tmc.tf_mc.tf.embedFonts = true;
			tmc.tf_mc.tf.styleSheet = css;
			tc.addChild(tmc);
			...
			tmc2.name = "textMC";
			tmc2.tf_mc.tf.x = textDistance;
			tmc2.tf_mc.tf.y = textDistance;
			tmc2.tf_mc.tf.width = imageHeight - textDistance * 2;
			tmc2.tf_mc.tf.height = imageHeight - textDistance * 2;
			tmc2.tf_mc.tf.wordWrap = true;
			tmc2.tf_mc.tf.mouseWheelEnabled = false;
			tmc2.tf_mc.tf.embedFonts = true;
			tmc2.tf_mc.tf.styleSheet = css;
		...
		private function loadImage () {
			...
			loader.load(new URLRequest(loadingList[loadCounter]));
		}
		...
		private function clicked (e:MouseEvent) {
			...
			tc.tf_mc.tf.text = xml.Image[currentImage].Text.children().toXMLString().split("\n").join("").replace(/Ӂ/g, " ");
			MovieClip(singleText.getChildByName("textMC")).tf_mc.tf.text = xml.Image[currentImage].Text.children().toXMLString().split("\n").join("").replace(/Ӂ/g, " ");
			...
			if (autoplay != 0 && tc.tf_mc.tf.text != "") {
				autoplaymc.x = -30;
